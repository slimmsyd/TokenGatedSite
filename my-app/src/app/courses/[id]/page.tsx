...
useEffect(() => {
    console.log("Logign courses modules", course?.modules)
}, [course])
  ...
  const handleEditModule = (moduleId: number, field: string, currentContent: string) => {
  ...
  const handleSaveModule = async (moduleId: number) => {
    ...
    const updatedModules = course.modules.map(module => 
      module.id === moduleId ? { ...module, [editingField as keyof Module]: editedContent } : module
    );
...
{course && course.modules && course.modules.length && isAdminMode ? (
    
    course.modules.map((module) => (
        <div key={module.id} className="mb-8 p-4 border rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold mb-4">{module.title}</h2>
        
        {['videoURL', 'title', 'concepts', 'objectives', 'activities'].map((field) => (
          <div key={field} className="mb-4">
            <h3 className="text-xl font-medium mb-2">{field === 'videoURL' ? 'Video URL' : field.charAt(0).toUpperCase() + field.slice(1)}:</h3>
            {editingModuleId === module.id && editingField === field ? (
              <>
                <textarea
                  value={editedContent}
                ...
                />
                <button onClick={() => handleSaveModule(module.id)} className="bg-blue-500 text-white px-4 py-2 rounded mt-2">
                  Save
                </button>
              </>
            ) : (
              <>
                {field === 'videoURL' ? (
                  ...
                  <ul className="list-disc list-inside">
                    {(module[field as keyof Module] as string[]).map((item, i) => (
                      <li key={i} className="text-base">{item}</li>
                    ))}
                  </ul>
                )}
                <button onClick={() => handleEditModule(module.id, field, field === 'videoURL' || field === 'title' ? module[field as keyof Module] as string : (module[field as keyof Module] as string[]).join('\n'))} className="bg-gray-200 px-4 py-2 rounded mt-2">
